...
int main() {
    ...
    for (int process = 0; process < K; process++) {
        ...
        if (pid < 0) {
            ...
            int *ptr1_child = mmap(0, size1, PROT_WRITE, MAP_SHARED, shm_fd1_child, 0);

            int shm_fd2_child = shm_open(name2, O_RDWR, 0666);
            ftruncate(shm_fd2_child, size2);
            int *ptr2_child = mmap(0, size2, PROT_WRITE, MAP_SHARED, shm_fd2_child, 0);
            ...
            for (int i = 0; i < columns; i++) {
                ...
                if (num > 0) {
                    if (is_tetrahedral(num)) {
                        ...
                        ptr1_child[ptr2_child[0] + 1] = getpid(); // Store child process ID
                        ptr2_child[0] += 2;
                    } else {
